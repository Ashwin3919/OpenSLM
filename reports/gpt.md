...
Three ready-to-use model configs are provided in `configs/model/`.
...
```yaml
# configs/experiments/my_gpt_run.yaml
_includes_:
  - "../base.yaml"
...
```bash
python main.py prep     --config configs/experiments/my_gpt_run.yaml
python main.py train    --config configs/experiments/my_gpt_run.yaml
python main.py generate --config configs/experiments/my_gpt_run.yaml \
...
Defined in `configs/training/default.yaml`, deserialized into `TrainingConfig`.
...
| `checkpoint_path` | `outputs/checkpoints/` | Directory for `.pt` checkpoint files. |
...
Written to `training.checkpoint_path` (default `outputs/checkpoints/`). Each `.pt` file contains:
...
`outputs/metrics.json` is updated after each evaluation interval. Fields include `iter`, `train_loss`, `val_loss`, `lr`. Read by `notebooks/03_training_monitor.ipynb`.
