...
# Embedding Block
embed_y, embed_h = 8.8, 0.8
draw_box(ax, center_x, embed_y, box_width, embed_h,
         "wte: token embedding")
ax.text(center_x + box_width/2 + 0.3, embed_y, r"$(B, T, d_{model})$",
        ha='left', va='center', fontsize=12, family='serif')
draw_arrow(ax, center_x, embed_y - embed_h/2, center_x, 7.7)
...
draw_arrow(ax, center_x, tf_y - tf_h/2, center_x, 3.1)
...
draw_box(ax, center_x, head_y, box_width + 1.0, head_h,
         r"lm\_head: Linear  $(d_{model} \rightarrow vocab\_size)$",
         fontsize=11)
...
plt.tight_layout()
plt.savefig(out_dir / "mamba_architecture.png", format='png', bbox_inches='tight', dpi=300)
plt.close()
print(f"  Saved → {out_dir / 'mamba_architecture.png'}")
