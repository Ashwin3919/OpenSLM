...
draw_arrow(ax, center_x, embed_y - embed_h/2, center_x, 7.3)
...
draw_box(ax, center_x, tf_y, box_width + 0.4, tf_h,
         ...
         "CausalSelfAttention\n"
         "LayerNorm\n"
         r"MLP ($4\times$ expansion)",
         facecolor='#f8f9fa')
ax.text(center_x + box_width/2 + 0.5, tf_y + tf_h/2 - 0.2, r"$\times n_{layer}$",
        ha='left', va='center', fontsize=14, family='serif')
ax.text(center_x + box_width/2 + 0.5, tf_y - 0.2, "residual connection\naround each",
        ha='left', va='center', fontsize=11, family='serif', style='italic', color='#444444')
draw_arrow(ax, center_x, tf_y - tf_h/2, center_x, 3.2)

# Final LayerNorm
ln_y, ln_h = 2.8, 0.8
draw_box(ax, center_x, ln_y, box_width, ln_h, "LayerNorm (final)")
...
plt.tight_layout()
plt.savefig(out_dir / "gpt_architecture.png", format='png', bbox_inches='tight', dpi=300)
plt.close()
print(f"  Saved → {out_dir / 'gpt_architecture.png'}")
