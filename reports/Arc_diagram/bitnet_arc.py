...
# Embedding Block  (full precision — not quantized)
embed_y, embed_h = 8.8, 1.0
draw_box(ax, center_x, embed_y, box_width, embed_h,
         "wte: token embedding  [full precision — not quantized]\nDropout")
...
draw_box(ax, center_x, head_y, box_width + 1.0, head_h,
         r"lm\_head: Linear  $(n_{embd} \rightarrow vocab\_size)$  [full precision]",
         fontsize=11)
...
plt.tight_layout()
plt.savefig(out_dir / "bitnet_architecture.png", format='png', bbox_inches='tight', dpi=300)
plt.close()
print(f"  Saved → {out_dir / 'bitnet_architecture.png'}")
