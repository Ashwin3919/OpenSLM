...
# Input
ax.text(center_x, 13.2, r"Input token IDs $(B, T)$",
        ha='center', va='center', fontsize=12, family='serif')
draw_arrow(ax, center_x, 12.9, center_x, 12.4)

# Embedding Block  (no wpe — token shift + WKV decay provide context)
embed_y, embed_h = 12.0, 0.8
...
draw_arrow(ax, center_x, embed_y - embed_h/2, center_x, 11.0)
...
draw_box(ax, center_x, tf_y, box_width + 0.4, tf_h,
         ...
         "RMSNorm\n"
         "Token shift\n"
         "WKV recurrence  +  Receptance gate\n"
         "\n"
         "ChannelMix\n"
         "RMSNorm\n"
         "Squared-ReLU FFN",
...
ax.text(center_x + box_width/2 + 0.5, tf_y - 0.5, "residual connection\naround each\nsub-block",
        ha='left', va='center', fontsize=11, family='serif', style='italic', color='#444444')
draw_arrow(ax, center_x, tf_y - tf_h/2, center_x, 3.0)

# Final RMSNorm
ln_y, ln_h = 2.6, 0.8
draw_box(ax, center_x, ln_y, box_width, ln_h, "RMSNorm (final)")
draw_arrow(ax, center_x, ln_y - ln_h/2, center_x, 1.5)

# LM Head
head_y, head_h = 1.1, 0.8
...
plt.tight_layout()
plt.savefig(out_dir / "rwkv_architecture.png", format='png', bbox_inches='tight', dpi=300)
plt.close()
print(f"  Saved → {out_dir / 'rwkv_architecture.png'}")
