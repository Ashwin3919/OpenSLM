...
# Dense layers  [indices in dense_layers, e.g. [0, 1]]
dense_y, dense_h = 9.8, 3.6
draw_box(ax, center_x, dense_y, box_width + 0.4, dense_h,
         "DeepSeekBlock  [dense layers]\n\n"
         "RMSNorm\n"
         "GQAttention  +  RoPE\n"
         "RMSNorm\n"
         "SwiGLU FFN",
         facecolor='#f8f9fa', fontsize=11)
ax.text(center_x + box_width/2 + 0.5, dense_y + dense_h/2 - 0.2,
        r"$\times$ |dense layers|",
        ha='left', va='center', fontsize=13, family='serif')
ax.text(center_x + box_width/2 + 0.5, dense_y - 0.3, "residual connection\naround each",
        ha='left', va='center', fontsize=11, family='serif', style='italic', color='#444444')
draw_arrow(ax, center_x, dense_y - dense_h/2, center_x, 7.7)

# MoE layers  [all remaining layers]
moe_y, moe_h = 5.4, 4.2
draw_box(ax, center_x, moe_y, box_width + 0.4, moe_h,
         "DeepSeekBlock  [MoE layers]\n\n"
         "RMSNorm\n"
         "GQAttention  +  RoPE\n"
         "RMSNorm\n"
         ...
         facecolor='#f8f9fa', fontsize=11)
ax.text(center_x + box_width/2 + 0.5, moe_y + moe_h/2 - 0.2,
        r"$\times (n_{layer} -$ dense$)$",
        ha='left', va='center', fontsize=12, family='serif')
ax.text(center_x + box_width/2 + 0.5, moe_y - 0.4, "residual connection\naround each",
        ha='left', va='center', fontsize=11, family='serif', style='italic', color='#444444')
draw_arrow(ax, center_x, moe_y - moe_h/2, center_x, 2.9)

# Final RMSNorm
ln_y, ln_h = 2.5, 0.8
draw_box(ax, center_x, ln_y, box_width, ln_h, "RMSNorm (final)")
draw_arrow(ax, center_x, ln_y - ln_h/2, center_x, 1.4)

# LM Head
head_y, head_h = 1.0, 0.8
...
draw_box(ax, center_x, logits_y, box_width + 1.0, logits_h,
         r"Logits $(B, T, vocab\_size)$  [training  +  aux loss]" + "\n" +
...
plt.tight_layout()
plt.savefig(out_dir / "deepseek_architecture.png", format='png', bbox_inches='tight', dpi=300)
plt.close()
print(f"  Saved → {out_dir / 'deepseek_architecture.png'}")
